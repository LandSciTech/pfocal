...
.gaussian_strip_radius <- function(radius, sd=1, tail_included=TRUE){
  if(!is.logical(tail_included)){
    ...
    warning(paste0("radius should be an even multiple of 1. It will be ",
...
.gaussian_quarter_kernel_radius <- function(vertical_radius, vertical_sd=1, 
  ...
  return(.gaussian_strip_radius(vertical_radius,   sd=vertical_sd,   
                                tail_included=tail_included) 
    %*% 
       t(.gaussian_strip_radius(horizontal_radius, sd=horizontal_sd, 
                                tail_included=tail_included)))
}
...
.gaussian_quarter_kernel_confidence <- function(vertical_r0=0.05, vertical_sd=1, 
  ...
  return(.gaussian_strip_confidence(r0=vertical_r0,   sd=vertical_sd,   
                                    tail_included=tail_included) 
    %*% 
       t(.gaussian_strip_confidence(r0=horizontal_r0, sd=horizontal_sd, 
                                    tail_included=tail_included)))
}
