...
.gaussian_strip_confidence <- function(r0=0.05, sd=1, tail_included=TRUE){
  ...
  .gaussian_strip_radius(ceiling(stats::qnorm((r0/2), sd=sd, lower.tail=FALSE)-0.5), sd=sd, tail_included=tail_included)
}

.gaussian_quarter_kernel_radius <- function(vertical_radius, vertical_sd=1, horizontal_radius=vertical_radius, horizontal_sd=vertical_sd, tail_included=TRUE){
  return(.gaussian_strip_radius(vertical_radius,   sd=vertical_sd,   tail_included=tail_included) 
    %*% 
       t(.gaussian_strip_radius(horizontal_radius, sd=horizontal_sd, tail_included=tail_included)))
}
...
gaussian_kernel_radius <- function(
    vertical_radius, 
    vertical_sd=1, 
